...
The system uses **12 fixed palettes** (Matugen-free) defined as JSON in `config/hypr/scripts/quickshell/dock/palettes/`. The active palette is controlled from the Settings panel (`SUPER + SHIFT + D` → *Palette* card) and stored in `settings.json` under `dock.palette`.

## Palette files

- `dock/palettes/<slug>.json` — one file per palette (base-16 colors + optional `background`/`foreground`/`roles` overrides).
- `dock/palettes/index.json` — ordered list of palettes shown in the Settings panel.
...
1. User picks a palette (or border color) in the Settings panel.
2. `BarEditor` writes `settings.json` → `dock` (palette slug, border overrides).
3. Each widget's `Colors` instance watches `settings.json` and re-applies the palette live.
4. The dock bar and Settings panel push window-border colors to Hyprland **live** via `hyprctl eval 'hl.config({ general = { col = { active_border = "rgba(...)", inactive_border = "rgba(...)" } } })'` — no window restart needed.
...
The Settings panel can recolor the **active** palette in real time — no Matugen, no new palette format:

1. Open the Settings panel (`SUPER + SHIFT + D`) → *Palette* card → **Edit colors**.
...
Settings panel → Palette → Edit colors or directly; it hot-reloads through the
