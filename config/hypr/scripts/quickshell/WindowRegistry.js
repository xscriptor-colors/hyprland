...
function getLayout(name, mx, my, mw, mh, userScale) {
    ...
    let base = {
        ...
        "bar-editor": { w: Math.min(s(1800, scale), mw - s(40, scale)), h: s(760, scale), rx: Math.floor((mw - Math.min(s(1800, scale), mw - s(40, scale))) / 2), ry: Math.floor((mh/2)-(s(760, scale)/2)), comp: "dock/BarEditor.qml" },
